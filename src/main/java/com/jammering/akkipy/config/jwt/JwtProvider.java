...
public class JwtProvider {
    ...
    private final long JWT_GUEST_TOKEN_EXPIRATION=1000 * 60 * 5;
    ...
    public TokenResponse.ToKenInfo generateTemporaryToken(String socialId, Provider provider) {
        ...
        claims.put("authority", Authority.GUEST);
        claims.put("provider", provider.name());
        claims.put("signup", false);

        return createToken(claims, JWT_GUEST_TOKEN_EXPIRATION, Authority.GUEST);
    }
    ...
    public TokenResponse.ToKenInfo generateToken(Long userId, Role role) {
        ...
        claims.put("authority", Authority.USER);
        claims.put("signup", true);

        return createToken(claims, JWT_ACCESS_TOKEN_EXPIRATION,Authority.USER );
    }

    private TokenResponse.ToKenInfo createToken(Claims claims, long expireTime, Authority authority) {
        Date now = new Date();
        ...
        String refreshToken = Jwts.builder()
                .setClaims(claims)
                .setIssuedAt(now)
                .setExpiration(new Date(now.getTime() + expireTime *2 )) // Refresh token은 2배로 설정
                .signWith(SignatureAlgorithm.HS256, SECRET_KEY.getBytes())
                .compact();
        return new TokenResponse.ToKenInfo(BEARER_TYPE,accessToken,refreshToken , expireTime * 2, authority);
    }
